...
import secrets as kittntabbr_ai_secrets
...
class TestLoadFromEnvFile:
    def test_parses_key_from_env_file(self, tmp_path, monkeypatch):
        ...
        with patch.object(kittntabbr_ai_secrets, "_ENV_FILE", env_file):
            result = kittntabbr_ai_secrets._load_from_env_file()

        assert result == "sk-ant-from-file"

    def test_returns_none_when_file_missing(self, tmp_path):
        with patch.object(kittntabbr_ai_secrets, "_ENV_FILE", tmp_path / ".env"):
            result = kittntabbr_ai_secrets._load_from_env_file()
...
class TestLoadFromKeychain:
    def test_returns_none_on_non_macos(self, monkeypatch):
        ...
        assert kittntabbr_ai_secrets._load_from_keychain() is None
    ...
    def test_returns_key_on_successful_security_call(self, monkeypatch):
        ...
        with patch("subprocess.run", return_value=mock_result):
            result = kittntabbr_ai_secrets._load_from_keychain()
...
class TestLoad:
    def test_returns_immediately_when_env_var_set(self, monkeypatch):
        ...
        with patch.object(kittntabbr_ai_secrets, "_load_from_keychain") as mock_kc:
            kittntabbr_ai_secrets.load()
    ...
    def test_sets_env_var_from_env_file(self, tmp_path, monkeypatch):
        ...
        with patch.object(kittntabbr_ai_secrets, "_ENV_FILE", env_file):
            with patch.object(kittntabbr_ai_secrets, "_load_from_keychain", return_value=None):
                kittntabbr_ai_secrets.load()
    ...
    def test_prefers_keychain_over_env_file(self, tmp_path, monkeypatch):
        ...
        with patch.object(kittntabbr_ai_secrets, "_ENV_FILE", env_file):
            with patch.object(
                kittntabbr_ai_secrets, "_load_from_keychain", return_value="sk-ant-from-keychain"
            ):
                kittntabbr_ai_secrets.load()
    ...
    def test_raises_system_exit_when_no_key_found(self, monkeypatch):
        monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
        with patch.object(kittntabbr_ai_secrets, "_load_from_keychain", return_value=None):
            with patch.object(kittntabbr_ai_secrets, "_load_from_env_file", return_value=None):
